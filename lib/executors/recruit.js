...
const executors = ({ senders }) => {
  ...
  const executeArmyRecruitment = async () => {
    ...
    if (currentRecruitmentInstructionIndex % 3 === 0) await delay(roundRandomWithRange({ min: 1, randomize: 3 }) * 1000)
  ...
  return ({
    ...
    [CHARACTER_INFO_TYPE_RECEIVE]: ({ response, config }) => {
      if (!config.recruit) return console.log('Not called for recruiting')
