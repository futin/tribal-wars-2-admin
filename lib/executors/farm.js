...
const executors = ({ senders }) => {
  ...
  const executePresetAttack = async () => {
    ...
    if (currentBarbarianIndex % 3 === 0) await delay(roundRandomWithRange({ min: 1, randomize: 3 }) * 1000)
  ...
  return ({
    [AUTH_LOGIN_SUCCESS_TYPE_RECEIVE]: ({ response }) => {
      const characterId = _.get(response, 'data.characters.0.character_id')
      const worldId = _.get(response, 'data.characters.0.world_id')

      senders.sendAuthCharSelectData({ characterId, worldId })
    },
    [CHARACTER_INFO_TYPE_RECEIVE]: ({ response, config }) => {
      if (!config.farm) return console.log('Not called for farming')
    ...
    [MAP_VILLAGES_TYPE_RECEIVE]: ({ response, config }) => {
      const { barbarianVillageNamesToIgnore } = config
      ...
      let barbarianVillages = villages
      ...
      if (barbarianVillageIdsToIgnore && barbarianVillageIdsToIgnore.length) {
        barbarianVillages = barbarianVillages.filter(({ id }) => !barbarianVillageIdsToIgnore.includes(id))
      }

      if (barbarianVillageNamesToIgnore && barbarianVillageNamesToIgnore.length) {
        barbarianVillages = barbarianVillages.filter(({ name }) => !barbarianVillageNamesToIgnore.includes(name))
      }
