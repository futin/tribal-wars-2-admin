...
import {
  ...
  moveSpecificUnitsExecutors, tradingResourcesExecutors, resourceDepositExecutors
...
const tribalWarsExecutors = async ({ config, mode, emitterKey }) => {
  ...
  if (mode === FARMING_MODE) {
    ...
    executors = farmingExecutors({ senders, emitterKey })
  }
  ...
  if (mode === ATTACK_SCHEDULER_MODE) {
    ...
    executors = attackSchedulerExecutors({ senders, emitterKey })
  }
  ...
  if (mode === RECRUIT_SPIES_MODE) {
    ...
    executors = recruitmentSpiesExecutors({ senders, emitterKey })
  }
  ...
  if (mode === MOVE_SPECIFIC_UNITS_MODE) {
    ...
    executors = moveSpecificUnitsExecutors({ senders, emitterKey })
  }
  if (mode === MASS_DONATION_MODE) {
    config = _.merge({ isMassDonation: true }, config)
    executors = massDonationExecutors({ senders, emitterKey })
  }
  if (mode === TRADING_RESOURCES_MODE) {
    config = _.merge({ isTradingResources: true }, config)
    executors = tradingResourcesExecutors({ senders, emitterKey })
  }
  if (mode === RESOURCE_DEPOSIT_MODE) {
    config = _.merge({ isResourceDeposit: true }, config)
    executors = resourceDepositExecutors({ senders, emitterKey })
  }
