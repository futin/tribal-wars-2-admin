...
import {
  SOCKET_URL, SOCKET_OPTS, DEFAULT_CONFIG, VALID_TW_MODES, ATTACK_MODE, BUILD_MODE, ATTACK_SCHEDULER_CALCULATION_MODE,
  ATTACK_SCHEDULER_MODE, RECRUIT_UNITS_MODE, RECRUIT_SPIES_MODE, VILLAGES_DATA_MODE, MOVE_UNITS_MODE,
  MASS_DONATION_MODE
} from '../constants'

import {
  farmExecutors, attackSchedulerCalculationExecutors, attackSchedulerExecutors, buildExecutors,
...
const tribalWarsAdmin = async ({ config, mode, emitter }) => {
  ...
  if (mode === BUILD_MODE) {
    config = _.merge({ isBuild: true }, config)
    executors = buildExecutors({ senders })
  }
  ...
  if (mode === RECRUIT_UNITS_MODE) {
    ...
    executors = recruitmentUnitsExecutors({ senders })
  }
