...
import { transformTargetCoordinates } from './utils'
...
const executors = ({ senders }) => {
  ...
  const executeMoveUnits = async () => {
    const { moveInstructions, moveInstructionsIndex } = executionInstance
    ...
    const { sourceVillageId, targetVillageId, type } = moveInstruction

    console.log(`Moving(${type}) units from [${sourceVillageId}] to [${targetVillageId}]
  ...
  return {
    ...
    [VILLAGES_DATA_TYPE_RECEIVE]: ({ response, config }) => {
      ...
      if (!allowedMoveTypes.includes(type)) {
        return senders.close('Move type not allowed')
      }
