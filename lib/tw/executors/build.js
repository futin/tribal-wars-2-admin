...
const executors = ({ senders }) => {
  ...
  async function prepareNextBuildingQueue (config) {
    ...
    senders.senBuildingUpgrade({ villageId: selectedVillage.id, building: 'wall' })
  }
  ...
  return ({
    ...
    [GAME_DATA_RECEIVE]: ({ response, config }) => {
      ...
      prepareNextBuildingQueue(config)
    },
    [BUILDING_UPGRADE_RECEIVE]: ({ response, config }) => {
      ...
      if (building && villageId === selectedVillage.id) {
        ...
        return prepareNextBuildingQueue(config)
      }
    },
