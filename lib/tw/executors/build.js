...
const executors = ({ senders }) => {
  ...
  function compileNextBuildingQueue (config) {
    const { selectedVillage, resources, queue, unlockedSlots, buildingsWithNextLevelCost } = executionInstance
    const { buildingOrder, buildingPriority } = config

    if (queue.length === unlockedSlots) return senders.close(`There are no free slots for village [${selectedVillage.name}] (2)`)

    console.log('---', require('util').inspect({ buildingOrder, buildingPriority, queue, resources, buildingsWithNextLevelCost }, true, null))
    return {}
  }
  ...
  return ({
    ...
    [GAME_DATA_RECEIVE]: ({ response, config }) => {
      const { selectedVillage, reducedVillageBuildings } = executionInstance
      ...
      const nextBuildingQueue = compileNextBuildingQueue(config)
      console.log(nextBuildingQueue)
      senders.close('Done')
    },
