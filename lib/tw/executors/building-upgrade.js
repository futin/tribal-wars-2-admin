...
const buildingOrderMap = {
  ...
  SECONDARY: ['hospital', 'tavern', 'statue', 'rallyPoint', 'market'],
...
function calculateNextBuildingUpgradeQueue ({ availableResources, buildingsWithNextLevelCost, buildingOrder, buildingPriority = 'RANDOM' }) {
  ...
  // console.log('---', require('util').inspect({ buildingUpgradesSorted, availableResources }, true, null))
...
const executors = ({ senders, emitterKey }) => {
  ...
  async function prepareNextBuildingUpgradeQueue (config) {
    ...
    if (queue.length === unlockedSlots) {
      return senders.close(`There are no available slots for village [${name}] (2)`, { emitterKey, payload: { queue } })
    }
    ...
    if (!nextBuildingUpgrade) {
      return senders.close(`There are no available building upgrades for village [${name}]`, { emitterKey, payload: { queue } })
    }
  ...
  return ({
    ...
    [VILLAGES_DATA_TYPE_RECEIVE]: ({ response }) => {
      ...
      if (queue.length === unlockedSlots) {
        return senders.close(`There are no available slots for village [${selectedVillage.name}]`, { emitterKey, payload: { queue } })
      }
