...
const executors = ({ senders, emitter }) => {
  ...
  if (emitter) emitter.hasData = true
  ...
  async function prepareNextBuildingUpgradeQueue (config) {
    ...
    if (queue.length === unlockedSlots) {
      if (emitter) emitter.emit('data', { queue })
      return senders.close(`There are no available slots for village [${name}] (2)`)
    }
    ...
    if (!nextBuildingUpgrade) {
      if (emitter) emitter.emit('data', { queue })
      return senders.close(`There are no available building upgrades for village [${name}]`)
    }
  ...
  return ({
    ...
    [VILLAGES_DATA_TYPE_RECEIVE]: ({ response }) => {
      ...
      if (queue.length === unlockedSlots) {
        if (emitter) emitter.emit('data', { queue })
        return senders.close(`There are no available slots for village [${selectedVillage.name}]`)
      }
