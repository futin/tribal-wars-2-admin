...
const executors = ({ senders }) => {
  ...
  const distanceCalculationAndAttackPlanner = () => {
    let { originVillage: { id, name, x, y }, targetVillagesDistances, unitType, arrivalDate, timezone = DEFAULT_TIMEZONE } = executionInstance
    console.log(`Calculation started for [${name}]`)

    arrivalDate = moment.utc(arrivalDate).tz(DEFAULT_TIMEZONE)
    const targets = targetVillagesDistances.map(target => {
      const { walkingDurations, name } = target
      const timeInSeconds = walkingDurations[unitType]

      if (!timeInSeconds) {
        console.log('Time not found for target', name)
        return
      }
      delete target.walkingDurations

      const launchTime = arrivalDate.clone().tz(timezone).add(-timeInSeconds, 'seconds').format(DATE_FORMAT)
      const travelTime = transformDuration(moment.duration(timeInSeconds, 'seconds'))
      return { ...target, launchTime, travelTime }
    })

    console.log('===================TARGETS\n\n')
    console.log({ id, name, targets, x, y })
    console.log('===================\n')
    senders.close('done')
  }
  ...
  return ({
    ...
    [MAP_VILLAGES_DETAILS_TYPE_RECEIVE]: ({ response }) => {
      ...
      executionInstance.targetVillagesDistances.push({ targetVillageName, walkingDurations, x, y })
      collectTargetVillagesData()
    },
