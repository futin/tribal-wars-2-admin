...
const executors = ({ senders }) => {
  const executionInstance = { isMoveSpecificUnits: true }
  ...
  return {
    ...
    [CHARACTER_INFO_TYPE_RECEIVE]: ({ response, config }) => {
      if (!config.isMoveSpecificUnits) return console.log('Not called for moving units')
    ...
    [COMMAND_SENT_PRESET_TYPE_RECEIVE]: async ({ response }) => {
      if (!executionInstance.isMoveSpecificUnits) return
