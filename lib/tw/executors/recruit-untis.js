...
const executors = ({ senders, emitter }) => {
  ...
  const executeArmyRecruitment = async () => {
    ...
    if (!recruitmentInstructions) {
      if (emitter) emitter.emit('close', { errorCode: 'InvalidInstructions' })
      return senders.close('Recruitment instructions not found! Please provide at least 1.', { playerVillagesLeft })
    }

    const recruitmentInstruction = recruitmentInstructions[currentRecruitmentInstructionIndex]

    if (!recruitmentInstruction) {
      if (emitter) emitter.emit('data', { timeCompleted })
  ...
  return ({
    ...
    [BARRACKS_RECRUIT_JOB_CREATED_TYPE_RECEIVE]: ({ response }) => {
      ...
      if (emitter) emitter.emit('close')
      return senders.close('No more recruitment jobs', { playerVillagesLeft })
    },
    ...
    [SYSTEM_ERROR_TYPE_RECEIVE]: ({ response }) => {
      ...
      if (errorCause === BARRACKS_RECRUIT_TYPE_SEND && errorCode === VILLAGE_INSUFFICIENT_RESOURCES_ERROR) {
        if (emitter) emitter.emit('close', { errorCode: 'InsufficientFunds' })
