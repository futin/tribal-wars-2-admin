...
const executors = ({ senders, emitterKey }) => {
  ...
  function prepareTradingResources (config) {
    ...
    const [source] = allSourcesVillagesWithDistances

    deepLog({ label: '---', value: source })
    deepLog({ label: '---config', value: config })
    const targets = VillageHandler.prepareNextTradingTargets({ sourceId: source.id })
