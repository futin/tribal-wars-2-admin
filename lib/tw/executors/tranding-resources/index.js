...
function calculateDistancesBetweenSourcesAndTargets ({ allSourceVillages, allTargetVillages }) {
  ...
  const allVillages = allSourcesVillagesWithDistances.concat(allTargetVillagesWithDistances)
  return { allVillages, allSourcesVillagesWithDistances, allTargetVillagesWithDistances }
}
...
const executors = ({ senders, emitterKey }) => {
  ...
  function prepareTradingResources (config) {
    const { allVillages, allSourcesVillagesWithDistances, allTargetVillagesWithDistances } = calculateDistancesBetweenSourcesAndTargets(executionInstance)
    const VillageHandler = new VillagesHandler({ villages: allVillages, config })

    const [source, source2] = allSourcesVillagesWithDistances

    const targets1 = VillageHandler.prepareNextTradingTargets({ sourceId: source.id })
    // const targets2 = VillageHandler.prepareNextTradingTargets({ sourceId: source2.id })
  ...
  return {
    ...
    [TRADING_MERCHANT_STATUS_RECEIVE]: ({ response, config }) => {
      ...
      if (availableMerchants <= 0) executionInstance.allSourceVillages = allSourceVillages.filter(({ id }) => id !== sourceVillage.id)
      else _.set(sourceVillage, 'availableMerchants', availableMerchants)
