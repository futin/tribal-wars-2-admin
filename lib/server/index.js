...
const { API_TOKEN, AUTH_NAME, AUTH_TOKEN, WORLD_NAME, GOOGLE_DOC_PRIVATE_KEY, GOOGLE_DOC_CLIENT_EMAIL, NODE_ENV } = process.env
...
const settings = {
  ...
  context: ({ req }) => {
    ...
    if (NODE_ENV === 'production' && !(authorization && authorization.includes(API_TOKEN))) {
