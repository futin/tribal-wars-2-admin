...
// local modules
import { ATTACK_SCHEDULER_MODE } from '#global-constants'

const attackSchedulerMiddleware = ({ models, tribalWarsExecutors }) => async (req, res) => {
  const { auth } = req
  if (!auth) {
    console.log('attackSchedulerMiddleware(): Missing auth')
    return
  }
  ...
  tribalWarsExecutors({ config: { scheduledAttacks, auth }, mode: ATTACK_SCHEDULER_MODE })
...
export default attackSchedulerConfig
