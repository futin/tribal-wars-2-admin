...
async function executeAutomaticRecruitOperations ({ recruitsToExecute, executedAt, models, auth, tribalWarsExecutors }) {
  ...
  config.auth = auth
  tribalWarsExecutors({ config, emitterKey: id, mode: RECRUIT_UNITS_MODE })
    ...
    setOneTimeListener({
      ...
      listener: async ({ isError, errorCode, payload }) => {
        if (isError || errorCode || !payload) {
          ...
          if (minimumRetrialIntervalInMinutes) {
            nextExecutionAt = moment.utc().add(minimumRetrialIntervalInMinutes, 'minutes').toDate()
