...
const schedulerMiddleware = ({ models, tribalWarsExecutors }) => async (req, res) => {
  // end immediately, without waiting for all the executions
  res.end('Done')
  const { auth, accountId } = req
  if (!auth) {
    console.log('schedulerMiddleware(): Missing auth')
    return
  }
  const executedAt = moment.utc().toDate()
  const queryExecutionDue = { nextExecutionAt: { $lt: executedAt }, isInvalid: null, accountId }
