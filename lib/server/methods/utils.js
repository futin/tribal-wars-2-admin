...
// local modules
import { DATE_FORMAT, DEFAULT_TIMEZONE, BACKGROUND_CELL_COLORS } from '#global-constants'

const transformAttackScheduler = attackScheduler => {
...
const initGoogleDoc = async ({ googleDocAuth, googleDocId }) => {
...
const setupSheet = async ({ googleDoc, title, headerValues }) => {
...
const buildAverageFormula = ({ char, min = 2, max }) => `=ROUNDUP(AVERAGE(${char}${min}:${char}${max}))`

const buildAverageCell = async ({ sheet, data, headerValues }) => {
...
const addColorToCellRelatedToAverage = async ({ sheet, data, headerValues }) => {
  await sheet.loadCells()
...
const addColorToCellRelatedToStorage = async ({ sheet, data, headerValues }) => {
  await sheet.loadCells()
  ...
  cells.forEach((cell, index) => {
    ...
    if (difference <= 0.2) cell.textFormat = { foregroundColor: BACKGROUND_CELL_COLORS.GRAY }
    if (difference > 0.2 && difference <= 0.6) cell.textFormat = { foregroundColor: BACKGROUND_CELL_COLORS.GREEN }
...
export {
  transformAttackScheduler,
  initGoogleDoc,
  setupSheet,
  sortVillages,
  transformVillageData,
  buildAverageCell,
  addColorToCellRelatedToAverage,
  addColorToCellRelatedToStorage
}
