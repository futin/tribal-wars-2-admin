...
export const addBackgroundColorToAllRows = async ({ sheet, data, headerValues }) => {
  await sheet.loadCells()

  const collMax = headerValues.length - 1
  const rawMax = data.length - 1
  ...
  cells.forEach((cell, index) => {
    if (cell._row % 2 !== 0) {
      cell.backgroundColor = BACKGROUND_CELL_COLORS.LIGHT_PURPLE
    }
  })
  await sheet.saveUpdatedCells()
}
